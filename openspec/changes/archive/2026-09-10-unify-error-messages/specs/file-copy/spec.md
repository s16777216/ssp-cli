...
- **THEN** system displays error `Error: Source is a folder, use -r` and exits with non-zero code
...
- **THEN** system prompts `Destination already exists, overwrite? [y/N]`, proceeds only on `y` (re-issues COPY with `Overwrite: T`)
...
- **THEN** system displays error `Error: Source not found - <src>` and exits with non-zero code
...
- **THEN** system displays `Copy complete: <dst>`